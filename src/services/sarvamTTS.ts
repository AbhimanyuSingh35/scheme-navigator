...
async function translateToHindi(text: string): Promise<string> {
  ...
  // Split into sentence-based chunks under 900 chars
  ...
  console.log(`Translating in ${chunks.length} chunks, total chars: ${text.length}`);
...
async function textToSpeech(text: string): Promise<Buffer> {
  ...
  console.log(`TTS: ${chunks.length} chunk(s), total chars: ${text.length}`);
...
export async function convertGuideToHindi(
  ...
  console.log("Step 1 - English text length:", english.length);
  console.log("Step 1 - Preview:", english.substring(0, 150));

  const hindi = await translateToHindi(english);
  console.log("Step 2 - Hindi translation length:", hindi.length);
  console.log("Step 2 - Preview:", hindi.substring(0, 150));
