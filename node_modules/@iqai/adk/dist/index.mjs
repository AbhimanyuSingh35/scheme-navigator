...
import { Type as Type22 } from "@google/genai";
...
function extractParametersSchema(func, ignoreParams = []) {
  ...
  if (!paramMatch) return { type: Type22.OBJECT, properties: {} };
  ...
  if (paramList.length === 0 || paramList.length === 1 && paramList[0] === "") {
    return { type: Type22.OBJECT, properties: {} };
  }
...
var FileStorageProvider = class {
  ...
  async loadIndex(userId, appName) {
    ...
    const appDirs = appName ? [appName.replace(/[^a-zA-Z0-9_-]/g, "_")] : readdirSync(userPath);
...
var FileVectorStore = class {
  ...
  constructor(config) {
    ...
    this.format = config.format ?? "json";
    this.indexPath = join2(this.basePath, "vectors.json");
    this.summariesPath = join2(this.basePath, "summaries");
    if (!existsSync2(this.basePath)) {
      mkdirSync2(this.basePath, { recursive: true });
    }
    if (this.writeSummaries && !existsSync2(this.summariesPath)) {
      mkdirSync2(this.summariesPath, { recursive: true });
    }
    this.loadIndex();
    this.logger.debug("Initialized", {
  ...
  clear() {
    ...
    this.saveIndex();
    if (this.writeSummaries && existsSync2(this.summariesPath)) {
      const files = __require("fs").readdirSync(this.summariesPath);
      for (const file of files) {
        try {
          unlinkSync2(join2(this.summariesPath, file));
        } catch {
        }
      }
    }
  ...
  loadIndex() {
    if (existsSync2(this.indexPath)) {
      try {
        const content = readFileSync2(this.indexPath, "utf-8");
  ...
  saveIndex() {
    writeFileSync2(this.indexPath, JSON.stringify(this.index, null, 2), "utf-8");
  }
  ensureIndex() {
    if (!this.index) {
      this.loadIndex();
  ...
  writeSummaryFile(id, metadata) {
    ...
    const filePath = join2(this.summariesPath, `${datePrefix}_${safeId}.md`);
...
var QdrantVectorStore = class {
  ...
  async ensureInitialized() {
    ...
    let QdrantClient;
    try {
      ({ QdrantClient } = __require("@qdrant/js-client-rest"));
    } catch {
      throw new Error(
...
var GeminiContextCacheManager = class {
  ...
  generateCacheFingerprint(llmRequest, cacheContentsCount) {
    ...
    return crypto2.createHash(Fingerprint.ALGORITHM).update(fingerprintStr).digest("hex").substring(0, Fingerprint.LENGTH);
  }
...
var GoogleLlm = class extends BaseLlm {
  ...
  get apiClient() {
    if (!this._apiClient) {
      ...
      const location = process.env.GOOGLE_CLOUD_LOCATION;
      if (useVertexAI && project && location) {
        this._apiClient = new GoogleGenAI2({
          vertexai: true,
          project,
          location
        });
  ...
  get liveApiClient() {
    if (!this._liveApiClient) {
      ...
      const location = process.env.GOOGLE_CLOUD_LOCATION;
      if (useVertexAI && project && location) {
        this._liveApiClient = new GoogleGenAI2({
          vertexai: true,
          project,
          location,
...
var googleSearchArgsSchema = z2.object({
  query: z2.string().min(1, "Query cannot be empty"),
  numResults: z2.number().int().min(1).max(10).default(5),
  start: z2.number().int().min(1).default(1),
  safe: z2.enum(["active", "off"]).default("off"),
  siteSearch: z2.string().optional(),
  siteSearchFilter: z2.enum(["i", "e"]).optional(),
  lr: z2.string().optional(),
  cr: z2.string().optional(),
...
import { Type as Type12 } from "@google/genai";
...
init_base_tool();
import { Type as Type13 } from "@google/genai";
...
import { Type as Type14 } from "@google/genai";
...
var GrepTool = class extends BaseTool {
  ...
  getDeclaration() {
    return {
      ...
      parameters: {
        type: Type17.OBJECT,
        properties: {
          pattern: { type: Type17.STRING },
          files: { type: Type17.STRING },
          caseInsensitive: { type: Type17.BOOLEAN },
          contextLines: { type: Type17.NUMBER },
          maxMatches: { type: Type17.NUMBER },
          wholeWord: { type: Type17.BOOLEAN },
          invertMatch: { type: Type17.BOOLEAN },
          exclude: { type: Type17.STRING },
          include: { type: Type17.STRING }
        },
  ...
  async resolveFiles(filesPattern, excludeArg, includeArg, warnings) {
    ...
    const absPattern = path3.resolve(cwd, filesPattern);
    const files = [];
    let stat2;
    try {
      stat2 = await fs3.stat(absPattern);
    } catch {
      stat2 = void 0;
    }
  ...
  async walkDir(dir, out, exclude, include, matcher) {
    ...
    for (const e of entries) {
      ...
      const rel = path3.relative(process.cwd(), full).replace(/\\/g, "/");
  ...
  async searchFile(file, regex, contextLines, invertMatch, remaining) {
    const lines = (await fs3.readFile(file, "utf8")).split("\n");
    const matches = [];
...
var ReadTool = class extends BaseTool {
  ...
  async runAsync(args, _context) {
    try {
      const filePath = path4.resolve(process.cwd(), args.path);
      try {
  ...
  getDeclaration() {
    return {
      ...
      parameters: {
        type: Type20.OBJECT,
        properties: {
          query: { type: Type20.STRING, description: "Search terms" },
          searchDepth: {
            type: Type20.STRING,
    ...
    try {
      ...
      if (!parsed.success) {
        return {
          ...
          details: z3.treeifyError(parsed.error)
        };
      }
...
var webSearchArgsSchema = z3.object({
  query: z3.string().min(1, "Query cannot be empty"),
  searchDepth: z3.enum(["basic", "advanced", "fast", "ultra-fast"]).default("basic"),
  chunksPerSource: z3.number().int().min(1).max(3).default(3),
  maxResults: z3.number().int().min(1).max(20).default(5),
  topic: z3.enum(["general", "news", "finance"]).default("general"),
  timeRange: z3.enum(["day", "week", "month", "year", "d", "w", "m", "y"]).nullable().default(null),
  startDate: z3.string().nullable().default(null),
  endDate: z3.string().nullable().default(null),
  includeAnswer: z3.union([z3.boolean(), z3.enum(["basic", "advanced"])]).default(false),
  includeRawContent: z3.union([z3.boolean(), z3.enum(["markdown", "text"])]).default(false),
  includeImages: z3.boolean().default(false),
  includeImageDescriptions: z3.boolean().default(false),
  includeFavicon: z3.boolean().default(false),
  includeDomains: z3.array(z3.string()).default([]),
  excludeDomains: z3.array(z3.string()).default([]),
  country: z3.string().nullable().default(null),
  autoParameters: z3.boolean().default(false),
  includeUsage: z3.boolean().default(false)
});
var tavilySearchResultSchema = z3.object({
  title: z3.string(),
  url: z3.string().url(),
  content: z3.string(),
  raw_content: z3.string().nullable().optional(),
  favicon: z3.string().nullable().optional(),
...
var tavilySearchResponseSchema = z3.object({
  ...
  usage: z3.record(z3.any(), z3.string()).optional(),
...
import { Type as Type21 } from "@google/genai";
var WriteTool = class extends BaseTool {
  ...
  async runAsync(args, _context) {
    try {
      const filePath = path5.resolve(process.cwd(), args.path);
      try {
      ...
      await fs5.writeFile(filePath, args.content, "utf8");
      return {
        success: true,
...
import { Type as Type23 } from "@google/genai";
...
function normalizeJsonSchema(schema) {
  if (!schema) {
    return { type: Type23.OBJECT, properties: {} };
  }
  ...
  switch (normalizedSchema.type) {
    ...
    case "boolean":
      return { type: Type23.BOOLEAN };
    case "null":
      return { type: Type23.NULL };
    default:
...
function determineSchemaType(schema) {
  if (schema.properties || schema.required || schema.additionalProperties !== void 0) {
    return Type23.OBJECT;
  }
  if (schema.items) {
    return Type23.ARRAY;
  }
  if (schema.enum !== void 0) {
    if (schema.enum.length === 0) return Type23.STRING;
    const firstItem = schema.enum[0];
    if (typeof firstItem === "string") return Type23.STRING;
    if (typeof firstItem === "number") return Type23.NUMBER;
    if (typeof firstItem === "boolean") return Type23.BOOLEAN;
    return Type23.STRING;
  }
  ...
  if (schema.minimum !== void 0 || schema.maximum !== void 0 || schema.exclusiveMinimum !== void 0 || schema.exclusiveMaximum !== void 0) {
    return schema.multipleOf === void 0 || schema.multipleOf % 1 === 0 ? Type23.INTEGER : Type23.NUMBER;
  }
  return Type23.OBJECT;
}
...
var InstructionsLlmRequestProcessor = class extends BaseLlmRequestProcessor {
  async *runAsync(invocationContext, llmRequest) {
    ...
    if (agent.outputSchema) {
      try {
        const raw = z5.toJSONSchema(agent.outputSchema);
...
var pluginCallbackNameSchema = z6.enum([
...
var InMemorySessionService = class extends BaseSessionService {
  ...
  createSessionImpl(appName, userId, state, sessionId) {
    const finalSessionId = sessionId?.trim() || randomUUID3();
    const session = {
      appName,
      userId,
      id: finalSessionId,
...
var contextCacheConfigSchema = z7.object({
  /**
   * Maximum number of invocations to reuse the same cache before refreshing it.
   ...
   * @default 10
   */
  cacheIntervals: z7.number().int().min(1, { message: "cacheIntervals must be at least 1." }).max(100, { message: "cacheIntervals must not exceed 100." }).default(10),
  /**
   ...
   * @default 1800
   */
  ttlSeconds: z7.number().int().positive({ message: "ttlSeconds must be greater than 0." }).default(1800),
  /**
   * Minimum estimated request tokens required to enable caching.
   ...
   * @default 0
   */
  minTokens: z7.number().int().min(0, { message: "minTokens must be non-negative." }).default(0)
});
...
var ReflectAndRetryToolPlugin = class extends BasePlugin {
  ...
  _createToolReflectionResponse(tool, toolArgs, error, retryCount) {
    ...
    const reflectionMessage = dedent2`
...
var VertexAiSessionService = class extends BaseSessionService {
  ...
  getApiClient() {
    const { GoogleGenAI: GoogleGenAI3 } = __require("@google/genai");
...
export {
  ...
  SEMCONV,
  SafetyEvaluatorV1,
  scheduling_exports as Scheduling,
  SequentialAgent,
  ...
  adkToMcpToolType,
  requestProcessor as agentTransferRequestProcessor,
  requestProcessor3 as basicRequestProcessor,
  buildFunctionDeclaration,
  buildLlmRequestForTrace,
  buildLlmResponseForTrace,
  requestProcessor4 as codeExecutionRequestProcessor,
  ...
  initializeTelemetry,
  injectSessionState,
  requestProcessor8 as instructionsRequestProcessor,
  isArtifactRef,
  ...
  requestProcessor9 as nlPlanningRequestProcessor,
