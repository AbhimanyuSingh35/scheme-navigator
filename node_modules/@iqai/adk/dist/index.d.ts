 ...
 * ```
 */
declare class CohereEmbeddingProvider implements EmbeddingProvider {
     ...
     */
    static forQuery(config?: Omit<CohereEmbeddingProviderConfig, "inputType">): CohereEmbeddingProvider;
    /**
     * Create a provider configured for documents.
     * Use this when embedding documents for storage.
     */
    static forDocument(config?: Omit<CohereEmbeddingProviderConfig, "inputType">): CohereEmbeddingProvider;
}
 ...
 * ```
 */
declare class OpenRouterEmbeddingProvider implements EmbeddingProvider {
    private readonly apiKey;
    private readonly model;
    private readonly baseUrl;
...
declare class MemoryService {
    ...
    private readonly summaryProvider?;
     ...
     */
    addSessionToMemory(session: Session, options?: {
     ...
     */
    search(query: Omit<MemorySearchQuery, "queryEmbedding" | "limit"> & {
        limit?: number;
     ...
     */
    count(filter: MemoryDeleteFilter): Promise<number | undefined>;
    /**
 ...
 * ```
 */
declare class FileStorageProvider implements MemoryStorageProvider {
     ...
     */
    private readRecordFromFile;
    /**
     * Convert a memory record to Markdown format.
     */
    private recordToMarkdown;
    /**
 ...
 * ```
 */
declare class VectorStorageProvider implements MemoryStorageProvider {
    private readonly vectorStore;
    private readonly searchMode;
    private readonly hybridWeights;
 ...
 * ```
 */
declare class FileVectorStore implements VectorStore {
    private readonly basePath;
    private readonly writeSummaries;
    private readonly format;
    private readonly logger;
    private readonly indexPath;
    private readonly summariesPath;
    private index;
 ...
 */
declare class InMemoryStorageProvider implements MemoryStorageProvider {
    private memories;
    /**
     * Store a memory record.
     */
...
declare class QdrantVectorStore implements VectorStore {
    ...
    private readonly createCollectionIfNotExists;
 ...
 * ```
 */
declare class LlmSummaryProvider implements MemorySummaryProvider {
    private readonly model;
 ...
 * ```
 */
declare class PassthroughSummaryProvider implements MemorySummaryProvider {
    /**
...
declare abstract class BaseSessionService {
     ...
     */
    abstract endSession(appName: string, userId: string, sessionId: string): Promise<Session | undefined>;
    /**
...
declare abstract class BaseTool {
     ...
     */
    runAsync(_args: Record<string, any>, _context: ToolContext): Promise<any>;
    /**
 ...
 */
declare class PreloadMemoryTool extends BaseTool {
    protected logger: Logger;
    private maxMemories;
    /**
 ...
 */
declare class RecallMemoryTool extends BaseTool {
    protected logger: Logger;
    /**
    ...
    runAsync(args: {
        query: string;
        limit?: number;
    }, context: ToolContext): Promise<RecallMemoryResult>;
}
 ...
 * ```
 */
declare class ForgetMemoryTool extends BaseTool {
    protected logger: Logger;
    constructor();
    /**
     * Get the function declaration for the tool.
    ...
    }, context: ToolContext): Promise<ForgetMemoryResult>;
}
 ...
 * ```
 */
declare class GetSessionDetailsTool extends BaseTool {
    protected logger: Logger;
    constructor();
    /**
     * Get the function declaration for the tool.
    ...
    runAsync(args: {
        sessionId: string;
    }, context: ToolContext): Promise<GetSessionDetailsResult>;
}
 ...
 * ```
 */
declare class WriteMemoryTool extends BaseTool {
    protected logger: Logger;
    constructor();
    /**
     * Get the function declaration for the tool.
    ...
    runAsync(args: {
        ...
        category?: string;
        keyFacts?: string[];
    }, context: ToolContext): Promise<WriteMemoryResult>;
}
 ...
 * No required env vars.
 */
declare function McpLimitless(config?: McpServerConfig): McpToolset;
/**
 ...
 */
declare function McpKalshi(config?: McpServerConfig): McpToolset;
/**
 ...
 */
declare function McpOpinion(config?: McpServerConfig): McpToolset;
/**
 ...
 */
declare function McpDefillama(config?: McpServerConfig): McpToolset;
/**
 ...
 */
declare function McpDebank(config?: McpServerConfig): McpToolset;
/**
 ...
 */
declare class RateLimitError extends LlmError {
    readonly code = 429;
    constructor(message: string, options?: LlmErrorOptions);
    /**
     * Create a RateLimitError from a provider-specific error
     */
    static fromError(originalError: unknown, provider: string, model?: string): RateLimitError;
    /**
 ...
 */
type AgentBuilderWithSchema<T, M extends boolean = false> = {
    [K in keyof AgentBuilder<any, any>]: K extends "withOutputSchema" ? AgentBuilder<any, any>[K] : AgentBuilder<any, any>[K] extends (...args: any[]) => AgentBuilder<any, any> ? (...args: Parameters<AgentBuilder<any, any>[K]>) => AgentBuilderWithSchema<T, M> : K extends "build" | "buildWithSchema" | "ask" ? K extends "build" ? () => Promise<BuiltAgent<T, M>> : K extends "buildWithSchema" ? <U = T>() => Promise<BuiltAgent<U, M>> : K extends "ask" ? (message: string | FullMessage) => Promise<RunnerAskReturn<T, M>> : never : AgentBuilder<any, any>[K];
};
/**
...
declare class PlanReActPlanner extends BasePlanner {
     ...
     */
    buildPlanningInstruction(_readonlyContext: ReadonlyContext, _llmRequest: LlmRequest): string;
    /**
     * Processes the LLM response for planning
     */
    processPlanningResponse(_callbackContext: CallbackContext, responseParts: Part[]): Part[] | undefined;
    /**
...
declare class LangfusePlugin extends BasePlugin {
    private client;
    private traces;
    private agentSpans;
    private toolSpans;
    private generations;
    private lastEventByInvocation;
    private tokenUsage;
    private modelsUsed;
    private modelsUsedKeysByInvocation;
    ...
    private getAgentSpanKey;
    private getToolSpanKey;
    private getGenerationKey;
 ...
 */
declare class ModelFallbackPlugin extends BasePlugin {
    private readonly logger;
     ...
     */
    private state;
    constructor(fallbackModels: string[], maxRetries?: number, retryDelayMs?: number);
    onModelErrorCallback(params: {
        callbackContext: CallbackContext;
        llmRequest: LlmRequest;
        error: unknown;
...
declare const REFLECT_AND_RETRY_RESPONSE_TYPE = "ERROR_HANDLED_BY_REFLECT_AND_RETRY_PLUGIN";
type PerToolFailuresCounter = Record<string, number>;
declare enum TrackingScope {
...
declare class ReflectAndRetryToolPlugin extends BasePlugin {
    maxRetries: number;
    throwExceptionIfRetryExceeded: boolean;
    scope: TrackingScope;
    private _scopedFailureCounters;
    private _lock;
    constructor({ name, maxRetries, throwExceptionIfRetryExceeded, trackingScope, }?: ReflectAndRetryToolPluginOptions);
    afterToolCallback(params: {
        tool: BaseTool;
        toolArgs: Record<string, any>;
        toolContext: ToolContext;
    ...
    extractErrorFromResult(_params: {
        tool: BaseTool;
        toolArgs: Record<string, any>;
        toolContext: ToolContext;
        result: any;
...
declare class ToolOutputFilterPlugin extends BasePlugin {
    private filterModel;
    ...
    private needsFiltering;
    private countKeys;
    private extractSchema;
    private iterativeFilter;
    private generateJqFilter;
    private getInitialSystemPrompt;
    private getIterativeSystemPrompt;
    private buildInitialPrompt;
    private buildIterativePrompt;
    private extractJqCommand;
 ...
 */
type SchedulerEventType = "schedule:triggered" | "schedule:completed" | "schedule:failed" | "schedule:paused" | "schedule:resumed";
...
declare class InMemorySessionService extends BaseSessionService {
     ...
     * Ends a session and returns its final state.
     */
    endSession(appName: string, userId: string, sessionId: string): Promise<Session | undefined>;
    /**
     * @deprecated Please migrate to the async method.
     */
    endSessionSync(appName: string, userId: string, sessionId: string): Session | undefined;
 ...
 */
declare function textStreamFrom(events: AsyncIterable<Event>): AsyncGenerator<string, void, unknown>;
/**
 ...
 */
declare function collectTextFrom(events: AsyncIterable<Event>): Promise<string>;
/**
 ...
 */
declare function streamTextWithFinalEvent(events: AsyncIterable<Event>): {
...
declare class InMemoryRunner<T extends BaseAgent = BaseAgent> extends Runner<T> {
    ...
    constructor(agent: T, { appName, plugins, }?: {
        appName?: string;
        plugins?: BasePlugin[];
    });
}
